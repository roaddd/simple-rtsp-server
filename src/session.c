...
static int eventDel(struct clientinfo_st *ev)
{
    ...
    for(int i = 0; i < sizeof(ev->event_data) / sizeof(ev->event_data[0]); i++){
        if(ev->event_data[i] != NULL){
            LOG_WARN("[CLIENT_EVENT_DEL] del slot=%d event_data=%p fd=%d fd_type=%d events=0x%x user_data=%p client name=%s",
                     i, ev->event_data[i], ev->event_data[i]->fd, ev->event_data[i]->fd_type,
                     ev->event_data[i]->events, ev->event_data[i]->user_data, ev->session->filename);
            if(delEvent(ev->event_data[i]) < 0){
                LOG_ERROR("[CLIENT_EVENT_DEL] del failed slot=%d event_data=%p client name=%s",
                          i, ev->event_data[i], ev->session->filename);
                return -1;
            }
        }
    }
    LOG_WARN("[CLIENT_EVENT_DEL] end client name=%s session=%p", ev->session->filename, ev->session);
    return 0;
}
static int delClient(event_data_ptr_t *event_data){
    ...
    if(clientinfo == NULL || clientinfo->sd == INVALID_SOCKET){
        LOG_WARN("[CLIENT_DEL] invalid client event_data=%p client name=%s",
                 event_data, clientinfo ? clientinfo->session->filename : "unknown");
        return -1;
    }
    LOG_WARN("[CLIENT_DEL] client found client name=%s session=%p sd=%d session_count=%d",
             clientinfo->session->filename, clientinfo->session, clientinfo->sd,
             clientinfo->session ? clientinfo->session->count : -1);
    if(eventDel(clientinfo) < 0){ // Delete all listening sockets(TCP/UDP)，下次epoll_wait就不会再触发这个客户的事件了
        LOG_ERROR("[CLIENT_DEL] eventDel failed client name=%s event_data=%p", clientinfo->session->filename, event_data);
        return -1;
    }

#ifdef SESSION_DEBUG
    LOG_INFO("client:%d offline\n", clientinfo->sd);
#endif

    struct session_st *session = clientinfo->session;
    int count = 0;

    LOG_WARN("[CLIENT_DEL] clearClient begin client name=%s tcp socketfd=%d", session->filename, clientinfo->sd);
    ...
    LOG_WARN("[CLIENT_DEL] clearClient end client name=%s session=%p session_count=%d",
             session->filename, session, count);
    /*Change the total number of customer connections*/
    mthread_mutex_lock(&mut_clientcount);
    sum_client--;
#ifdef SESSION_DEBUG
    LOG_INFO("sum_client:%d\n", sum_client);
#endif
    ...
    LOG_WARN("[CLIENT_DEL] end client name=%s session=%p count=%d sum_client=%d",
             session->filename, session, count, sum_client);
    return 0;
}
#ifdef RTSP_FILE_SERVER
...
int clearClient(struct clientinfo_st *clientinfo)
{
    ...
    LOG_WARN("[CLIENT_CLEAR] begin client name=%s session=%p sd=%d udp_rtp=%d udp_rtcp=%d udp_rtp_1=%d udp_rtcp_1=%d",
             clientinfo->session->filename, clientinfo->session, clientinfo->sd, clientinfo->udp_sd_rtp,
             clientinfo->udp_sd_rtcp, clientinfo->udp_sd_rtp_1, clientinfo->udp_sd_rtcp_1);
    if(clientinfo->sd != INVALID_SOCKET){
        LOG_INFO("[CLIENT_CLEAR] close tcp fd=%d client name=%s", clientinfo->sd, clientinfo->session->filename);
        closeSocket(clientinfo->sd);
        clientinfo->sd = INVALID_SOCKET;
    }
    if(clientinfo->udp_sd_rtp != INVALID_SOCKET){
        LOG_INFO("[CLIENT_CLEAR] close video_rtp fd=%d client name=%s", clientinfo->udp_sd_rtp, clientinfo->session->filename);
        closeSocket(clientinfo->udp_sd_rtp);
        clientinfo->udp_sd_rtp = INVALID_SOCKET;
    }
    if(clientinfo->udp_sd_rtcp != INVALID_SOCKET){
        LOG_INFO("[CLIENT_CLEAR] close video_rtcp fd=%d client name=%s", clientinfo->udp_sd_rtcp, clientinfo->session->filename);
        closeSocket(clientinfo->udp_sd_rtcp);
        clientinfo->udp_sd_rtcp = INVALID_SOCKET;
    }
    if(clientinfo->udp_sd_rtp_1 != INVALID_SOCKET){
        LOG_INFO("[CLIENT_CLEAR] close audio_rtp fd=%d client name=%s", clientinfo->udp_sd_rtp_1, clientinfo->session->filename);
        closeSocket(clientinfo->udp_sd_rtp_1);
        clientinfo->udp_sd_rtp_1 = INVALID_SOCKET;
    }
    if(clientinfo->udp_sd_rtcp_1 != INVALID_SOCKET){
        LOG_INFO("[CLIENT_CLEAR] close audio_rtcp fd=%d client name=%s", clientinfo->udp_sd_rtcp_1, clientinfo->session->filename);
        closeSocket(clientinfo->udp_sd_rtcp_1);
        clientinfo->udp_sd_rtcp_1 = INVALID_SOCKET;
    }
    for(int i = 0; i < sizeof(clientinfo->event_data) / sizeof(clientinfo->event_data[0]); i++){
        if(clientinfo->event_data[i] != NULL){
            LOG_WARN("[CLIENT_CLEAR] free event slot=%d event_data=%p fd=%d fd_type=%d events=0x%x user_data=%p client name=%s",
    ...
    LOG_WARN("[CLIENT_CLEAR] reset client fields client name=%s session=%p", clientinfo->session->filename, clientinfo->session);
    memset(clientinfo->client_ip, 0, sizeof(clientinfo->client_ip));
    clientinfo->client_rtp_port = -1;
    clientinfo->client_rtcp_port = -1;
    clientinfo->client_rtp_port_1 = -1;
...
int getVideoPacerStats(void *context, RtspVideoPacerStats *stats){
    ...
    for(i = 0; i < CLIENTMAX && reported < RTSP_VIDEO_PACER_STATS_MAX_CLIENTS; i++){
        ...
        stats->clients[reported].last_reset_lag_us = pacer_stats.last_reset_lag_us;
        stats->clients[reported].last_reset_now_us = pacer_stats.last_reset_now_us;
        stats->clients[reported].last_reset_next_send_ts_us = pacer_stats.last_reset_next_send_ts_us;
        stats->clients[reported].last_reset_window_bytes = pacer_stats.last_reset_window_bytes;
        stats->clients[reported].last_reset_window_packets = pacer_stats.last_reset_window_packets;
        reported++;
    }
